...
namespace game
{
    ...
    std::shared_ptr<mc::asset::ActLogic> BoxDraw::createFunc(mc::low::GameObject *gb, const std::string &bin_data)
    {
        auto res{std::make_shared<BoxDraw>()};
        res->m_gb = gb;
        res->maindraw.Create();
    ...
    {
        acf.Register("game::act_logic::BoxDraw", createFunc);
    }
