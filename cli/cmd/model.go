...
func _gencube(baseDir, bin_buffer string, x, y, z, px, py, pz float32) {
	copy_cube := make([]float32, 24*14)
	for idx := 0; idx < 24; idx++ {
		copy_cube[idx*14] = base_cube[idx*8]
		copy_cube[idx*14+1] = base_cube[idx*8+1]
		copy_cube[idx*14+2] = base_cube[idx*8+2]
		copy_cube[idx*14+3] = base_cube[idx*8+3]
		copy_cube[idx*14+4] = base_cube[idx*8+4]
		copy_cube[idx*14+5] = base_cube[idx*8+5]
		copy_cube[idx*14+6] = base_cube[idx*8+6]
		copy_cube[idx*14+7] = base_cube[idx*8+7]
	}
	// 修正 xyz
	for index := 0; index < len(copy_cube); index += 14 {
	...
	// 补充 T 和 B
	for idx := 0; idx < len(copy_cube); idx += 42 { // 三个点一算
		P1_x := copy_cube[idx+14] - copy_cube[idx]
		P1_y := copy_cube[idx+15] - copy_cube[idx+1]
		P1_z := copy_cube[idx+16] - copy_cube[idx+2]

		P2_x := copy_cube[idx+28] - copy_cube[idx]
		P2_y := copy_cube[idx+29] - copy_cube[idx+1]
		P2_z := copy_cube[idx+30] - copy_cube[idx+2]

		S1, T1 := copy_cube[idx+17]-copy_cube[idx+3], copy_cube[idx+18]-copy_cube[idx+4]
		S2, T2 := copy_cube[idx+31]-copy_cube[idx+3], copy_cube[idx+32]-copy_cube[idx+4]
		...
		copy_cube[idx+11], copy_cube[idx+12], copy_cube[idx+13] = Tx, Ty, Tz
		copy_cube[idx+11+14], copy_cube[idx+12+14], copy_cube[idx+13+14] = Tx, Ty, Tz
		copy_cube[idx+11+28], copy_cube[idx+12+28], copy_cube[idx+13+28] = Tx, Ty, Tz

		copy_cube[idx+8], copy_cube[idx+9], copy_cube[idx+10] = Bx, By, Bz
		copy_cube[idx+8+14], copy_cube[idx+9+14], copy_cube[idx+10+14] = Bx, By, Bz
		copy_cube[idx+8+28], copy_cube[idx+9+28], copy_cube[idx+10+28] = Bx, By, Bz
	}
	// ebo
...
func _gen(baseDir, bin_buffer, shape, parameter string) {
	if shape == "cube" {
